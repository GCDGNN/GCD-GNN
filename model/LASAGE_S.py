...
class LISeq(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        for i in range(1, len(self.modlist)):
            ...
            elif isinstance(self.modlist[i], GraphNorm): #根据整张图来进行计算
                out = self.modlist[i](out)
        return out
...
class SIMSeq(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        out, src = self.modlist[0](*args, **kwargs)
        for i in range(1, len(self.modlist)):
            if isinstance(self.modlist[i], SIMLinear):
                out, src = self.modlist[i](out, src)
            elif isinstance(self.modlist[i], nn.Dropout):
                out = self.modlist[i](out)
...
#         #需要训练的地方：mean和std聚合的时候的w可以训练，构建图的过程是固定的应该，代表mean和std也是受到分类loss影响的
#         pass
...
class LASAGE_S(nn.Module):
    ...
    def forward(self, blocks, relations, feats):   # blocks 
        ...
        for l, (layer, block) in enumerate(zip(self.layers, blocks)):  # blocks of one relation in one layer
            ...
            # dec_h = self.decode_relation_mlp[l](h)
            # decode_loss += self.dec_loss_fct(dec_h, relation_agg_emb)
            # h = layer(block, h)
        ...
        # return h_ori if self.ori else h
        return h




class SIMPMP(nn.Module):
    def __init__(self, in_size, 
	      			   hid_size, 
					   out_size, 
					   num_layers, 

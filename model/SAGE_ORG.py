	...
	def forward(self, graph, feat, edge_weight=None):
		...
		with graph.local_scope():
			...
			if edge_weight is not None:
				...
				graph.edata["_edge_weight"] = self.relu(edge_weight)
				graph.edata["_edge_weight_hete"] = self.relu(-edge_weight)
...
#ablation version
class GraphSAGE_DGL_ORG(nn.Module):
	def __init__(self, in_size, 
					   ...
					   agg = "mean", 
					   relation_agg = None):
		super(GraphSAGE_DGL_ORG, self).__init__()
		self.layers = nn.ModuleList()
		...
		self.tstconv = TSTConv(self.in_size,self.hid_size,self.out_size)
	...
	def forward(self, blocks, relations, feats):   # blocks 
		h = feats 
		feat_sim, simi_weight = self.tstconv(blocks[0],h)
		...
		blocks[0].srcdata['feature'] = feat_sim
...
#feat+attn
class GraphSAGE_DGL_ORG_feat_attn(nn.Module):
	def __init__(self, in_size, 
	      			   hid_size, 
					   out_size, 
					   num_layers, 
					   ...
					   agg = "mean", 
					   relation_agg = None):
		super(GraphSAGE_DGL_ORG_feat_attn, self).__init__()
		self.layers = nn.ModuleList()
		self.in_size = in_size
		self.hid_size = hid_size
		self.out_size = out_size
...
class GraphSAGE_Former(GraphSAGE_DGL_ORG):

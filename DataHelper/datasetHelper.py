...
#     train_nid = torch.LongTensor(torch.nonzero(train_mask, as_tuple=True)[0])
#     val_nid = torch.LongTensor(torch.nonzero(val_mask, as_tuple=True)[0])
#     test_nid = torch.LongTensor(torch.nonzero(test_mask, as_tuple=True)[0])
#     num_classes = dataset.num_classes
#     relations = list(data.etypes)
#     num_relations = len(data.etypes)
#     feat = data.ndata['feature']
#     feat_dim = feat.shape[1]
#     labels = data.ndata['label'].squeeze().long()
#     num_nodes = labels.shape[0]


def worker_init_fn(worker_id):
    np.random.seed(717 + worker_id)
...
class DatasetHelper(dataset):
    ...
    def load(self):
        ...
        # dataset = dgl.load_graphs("./comp.dgl")#直接硬改！！！！

        elif self.dataset_name == 'FDCompCN':
            dataset = dgl.load_graphs('./datasets/comp.dgl')[0]

        data = dataset[0]
        if self.config['add_relations']:
            from DataHelper.addMetaPath import process_yelp_metapath_mini
            data = process_yelp_metapath_mini(data)
        norm = self.config['norm_feat']
        # Feature tensor dtpye is float64, change it to float32
        data.ndata['feature'] = torch.from_numpy(self.row_normalize(data.ndata['feature'], dtype=np.float32)) if norm else data.ndata['feature'].float()   #normalised before agg
        if self.dataset_name == 'grab':
            labels = data.ndata['label']
            labels[labels==2] = 0
            data.ndata['label'] = labels
        if self.dataset_name in ['tfinance', 'tsocial', 'grab','FDCompCN']:
        ...
        self.config_data(data, dataset) #配置data
        if homo:
            ...
            self.train_loader, self.val_loader, self.test_loader = self.get_DGLloader(self.data,
                                                                                      sampler)  # 构建minibatch数据

        if self.config['model'] in ['GCDGNN', 'GCDGNN-L']:
            ...
            self.train_loader, self.val_loader, self.test_loader = self.get_DGLloader(self.data, sampler) #构建minibatch数据
    ...
    def config_data(self, data, dataset):
        ...
        self.val_nid         = torch.LongTensor(torch.nonzero(self.val_mask, as_tuple=True)[0])
        self.test_nid        = torch.LongTensor(torch.nonzero(self.test_mask, as_tuple=True)[0])
        self.num_classes     = dataset.num_classes if not self.dataset_name in ['tfinance', 'tsocial', 'grab','FDCompCN'] else len(torch.unique(data.ndata['label']))
